...
?> 
  ...
  <form class="formularz" action="Sudoku_testy.php" method="get"> 
...
<script>
  function solveSudoku(board) {
    ...
    function solve(pos = 0) {
      ...
      const col = pos % 9;
      if (board[row][col] !== '') return solve(pos + 1);
...
<script>
  QUnit.module("Sudoku Tests");

  QUnit.test("Auto-solve static puzzle and validate", function(assert) {
    ...
    setTimeout(() => {
      ...
      assert.strictEqual(message, "✅ Gratulacje! Rozwiązanie poprawne!", "Statyczna plansza rozwiązana poprawnie");
      done();
    }, 500);
  });

  QUnit.test("Auto-solve randomly generated puzzle and validate", function(assert) {
    const done = assert.async();

    const randomPuzzle = generatePuzzle('medium');
    renderBoard(randomPuzzle);

    setTimeout(() => {
      const copy = randomPuzzle.map(row => row.slice());
      ...
      checkSolution();
      const message = document.getElementById("message").textContent;
      assert.strictEqual(message, "✅ Gratulacje! Rozwiązanie poprawne!", "Losowa plansza rozwiązana poprawnie");
      done();
    }, 500);
  });
</script>
